...
const char * const BITCOIN_CONF_FILENAME = "quick.conf";
const char * const BITCOIN_PID_FILENAME = "quickd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "quick" is a composite category enabling all Quick-related debug output
            if(ptrCategory->count(string("quick"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\QuickCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\QuickCore
    // Mac: ~/Library/Application Support/QuickCore
    // Unix: ~/.quickcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "QuickCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/QuickCore";
#else
    // Unix
    return pathRet / ".quickcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty quick.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override quick.conf
